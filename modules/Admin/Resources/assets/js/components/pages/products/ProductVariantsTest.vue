...
function onEdit() {
    if (props.variants.length > 0) {
        const variantOption = [];
        props.variants.forEach(attrItem => {
            attrItem.values.forEach(variant => {
                variantOption.push({
                    attribute_name: variant.value,
                    attribute_value_id: variant.id,
                    product_id: attrItem.product_id,
        ...
        cartesianProducts.value.push(variantOption);
    }
}

watch(cartesianProducts, () => {
    let variants: any[] = cartesianProducts.value.map(cartesianProductItem => {
        const item = {name: null, sku: null, quantity: null, price: null, values: []};
        if (cartesianProductItem.length) {
            cartesianProductItem.forEach(cpItem => {
                if (!cpItem.name) {
                    item.name = cartesianProductItem.attribute_name;
                } else {
                    item.name += " + " + cartesianProductItem.attribute_name;
                }

                item.values.push(cartesianProductItem.attribute_value_id);

                if (cpItem?.sku) {
                    item.sku = cpItem.sku;
                }
                if (cpItem?.price) {
                    item.price = cpItem.price;
                }
                if (cpItem?.quantity) {
                    item.quantity = cpItem.quantity;
                }
            });
